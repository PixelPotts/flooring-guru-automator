...
import { Mic, MicOff, X, Loader, Bot, FileText, Calendar, Package2 } from 'lucide-react';
...
const VoiceAssistant: React.FC<VoiceAssistantProps> = ({ isOpen, onClose }) => {
  ...
  const handleCommand = async (command: string) => {
    ...
    try {
      ...
      setError(null);
      voiceCommandProcessor.updateContext(context);
      
      const result = await voiceCommandProcessor.processCommand(command);
      
      if (!result.success || !result.action) {
        await respondWithVoice(result.feedback || 'I did not understand that command. Please try again.');
        return;
      }

      setContext(prev => ({
        ...prev,
        currentProcess: result.action,
        collectedInfo: { ...prev.collectedInfo, ...result.parameters }
      }));

      if (result.action === 'navigate' && result.parameters?.path) {
        navigate(result.parameters.path);
        await respondWithVoice(`Navigating to ${result.parameters.path.replace('/', '')}`);
        return;
      }

      await respondWithVoice(result.feedback || 'Command processed successfully');

      if (result.action === 'create_client' && result.parameters) {
        addClient({
          name: result.parameters.name || '',
          company: result.parameters.company || '',
          email: result.parameters.email || '',
          phone: result.parameters.phone || '',
        ...
        voiceCommandProcessor.clearContext();
      }
