...
const InventorySelector: React.FC<InventorySelectorProps> = ({
  ...
  const handleSelect = (item: MaterialLineItem | LaborLineItem) => {
    onSelect(item);
    onClose();
  };
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-sm font-medium text-gray-900 dark:text-white">
                      ${activeTab === 'materials' 
                        ? (item as MaterialLineItem).default_unit_price?.toFixed(2)
                        : (item as LaborLineItem).default_hourly_rate?.toFixed(2)}/
                      {activeTab === 'materials' 
                        ? (item as MaterialLineItem).unit 
                        : 'hr'}
                    </span>
                  </div>
                </motion.button>
